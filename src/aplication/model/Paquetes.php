...
//        ?>
    ...
    public function updatePaquetes() {
        $image = "";
        if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != "") {
    ...
    public function deletePaquetes() {
        $this->deleteArchivo($_GET['id']);
    ...
    public function getPaquetes($actuales = FALSE) {
        if ($actuales) {
    ...
    public function newPaquetes() {
        ...
        <div id="steps">
            <div id="titu_step"> <span class="glyphicon glyphicon-calendar"></span>Crear Paquete <a class="btn btn_nuevo" href="cuenta.php?actividades=paquetes" title="listar Paquetes">Regresar</a></div>
            <div id="panel_step">
                ...
                <section  style="display:inline-block; width:67%; padding-top:10px; padding-left:10px ">
                    <form action="cuenta.php?actividades=paquetes&action=add" method="post" enctype="multipart/form-data" accept-charset="utf-8" name="form_datos" class="form-horizontal" id="form_datos" onsubmit="return validate2(this, 'add')" >
...
No Incluye:
						...
						<!--
                        <div class="form-group"><input type="text" name="itinerario_actividad_paquete" placeholder="itinerario_actividad_paquete" class="form-control" ></div>
                        <div class="form-group"><input type="text" name="inclusiones_actividad_paquete" class="form-control"  placeholder="inclusiones_actividad_paquete"></div>
                        <div class="form-group"><input type="text" name="no_incluye_actividad_paquete" placeholder="no_incluye_actividad_paquete" class="form-control" ></div>
                        -->
    ...
    public function addPaquetesCuenta(Cliente $cliente){
        ...
        $sql2 = "INSERT INTO actividades_paquetes(id_actividad,itinerario_actividad_paquete, inclusiones_actividad_paquete, no_incluye_actividad_paquete, dificultad_actividad_paquete)  
                                         VALUES ('" . $id_actividad . "','" . $_POST['itinerario_actividad_paquete'] . "','" . $_POST['inclusiones_actividad_paquete'] . "','" . $_POST['no_incluye_actividad_paquete'] . "','" . $_POST['dificultad_salida'] . "' )";
        $query2 = new Consulta($sql2);
        //echo $sql2; 
        ...
        //$this->_msgbox->setMsgbox('Se agregó correctamente.', 2);       
        location("cuenta.php?actividades=paquetes");
        
    }
    
    public function url_paquete($deporte, $id, $titulo) {
        $url_aventura = 'salidas-de-' . url_friendly($deporte, 1) . '/' . url_friendly($titulo, 1);
        return $url_aventura;
    }

    	
    public function editPaquetesCuenta(){
        settype($_GET['id'],'int');
        $data = new Paquete($_GET['id']);            
        ...
        <div id="steps">
            <div id="titu_step"> <span class="glyphicon glyphicon-calendar"></span>Editar Paquete <a class="btn btn_nuevo" href="cuenta.php?actividades=paquetes" title="Nuevo Evento">Regresar</a></div>
            <div id="panel_step">
               
                <form action="cuenta.php?actividades=paquetes&action=update&id=<?php echo $data->_id_salida;?>" method="post" enctype="multipart/form-data" accept-charset="utf-8" name="form_datos" id="form_datos" onsubmit="return validate2(this, 'update')">
                    <input type="hidden" value="update" name="action">
                    <input type="hidden" value="<?php echo $data->__get('_id_paquete');?>" name="id_actividad_paquete">
                    ...
                    <div id="div_input1">
                        ...
                        <!--
                        <div class="rowElem"><label>Itinerario:</label><input name="itinerario_actividad_paquete" id="itinerario_actividad_paquete" type="text" size="80" value="<?php echo $data->__get('_itinerario');?>"  /></div>
                        <div class="rowElem"><label>Inclusiones:</label><input name="inclusiones_actividad_paquete" id="inclusiones_actividad_paquete" type="text" size="80" value="<?php echo $data->__get('_inclusiones');?>"  /></div>
                        <div class="rowElem"><label>No Incluye:</label><input name="no_incluye_actividad_paquete" id="no_incluye_actividad_paquete" type="text" size="80" value="<?php echo $data->__get('_no_incluye');?>"  /></div>
                    	-->       
    ...
    public function updatePaquetesCuenta(Cliente $cliente){
        ...
        $user = $_SESSION['usuario']; 
            ...
            $query2 = new Consulta("UPDATE actividades_paquetes SET 
                                    itinerario_actividad_paquete='" . $_POST['itinerario_actividad_paquete'] . "',
                                    inclusiones_actividad_paquete='" . $_POST['inclusiones_actividad_paquete'] . "',
                                    no_incluye_actividad_paquete='" . $_POST['no_incluye_actividad_paquete'] . "',
                                    dificultad_actividad_paquete='" . $_POST['dificultad_salida'] . "'                                    
                                    WHERE id_actividad = '" . $_POST['id_actividad'] . "' ");
    ...
    public function statePaquetesCuenta(Cliente $cliente){
    ...
    public function deletePaquetesCuenta(Cliente $cliente) {

        $query1 = new Consulta("DELETE FROM actividades WHERE id_actividad = '" . $_GET['id'] . "'"); //Son los salidas por cliente Si estará ahi o no
        $query2 = new Consulta("DELETE FROM actividades_paquetes WHERE id_actividad = '" . $_GET['id'] . "' ");
    ...
    public function listPaquetes(Cliente $cliente){
         
        $salidas = $this->getPaquetesCuenta($cliente->__get("_id"));        
        $nsalidas = count($salidas);  ?>
        <div id="steps">
            <div id="titu_step"> <span class="glyphicon glyphicon-calendar"></span> Paquetes <a class="btn btn_nuevo" href="cuenta.php?actividades=paquetes&action=new" title="Nueva Salida">Nuevo Paquete</a></div>
            <div id="panel_step">
                ...
                <?php 
                    for($i = 0; $i < $nsalidas; $i++){
                      ...
                      <tr>
                          ...
                          <td> <?php echo $salidas[$i]['id_paquete'];?>   </td>
                        ...
                        <td>
                            <a href="cuenta.php?actividades=paquetes&action=inscritos&id_actividad=<?php echo $salidas[$i]['id_actividad']?>&id_tipo_actividad=<?php echo $salidas[$i]['id_tipo_actividad']?>" class="btn-circle btn-registro glyphicon glyphicon-th-list" title="Inscritos"> </a>                       
                                                  
                            <a href="cuenta.php?actividades=paquetes&action=edit&id=<?php echo $salidas[$i]['id_paquete']?>" class="btn-circle btn-edit glyphicon glyphicon-pencil" title="Editar"> </a>
                            <!--<a href="cuenta.php?actividades=paquetes&action=del&id=<?php echo $salidas[$i]['id_actividad']?>" class="btn-circle btn-delete glyphicon glyphicon-remove" title="Eliminar"></a>-->
                            <a href="cuenta.php?actividades=paquetes&action=state&id=<?php echo $salidas[$i]['id_actividad']?>&id_estado=<?php echo $salidas[$i]['estado']?>" class="btn-circle btn-info glyphicon glyphicon-remove-sign" title="Activar/Desactivar"></a>
                            
                            <a href="cuenta.php?actividades=salidas&action=estaditica&id=<?php echo $salidas[$i]['id_paquete']?>" class="btn-circle btn-reporte glyphicon glyphicon-stats" title="Estadistica"> </a>   
                        </td>
                      </tr>
    ...
    public function getPaquetesCuenta($id_cliente){
        $sql = "SELECT id_actividad,id_actividad_paquete, id_tipo_actividad, nombre_actividad, fecha_actividad, estado_actividad,lugar_actividad,
                    ( SELECT COUNT(i.id_inscripcion) FROM inscripciones i  WHERE i.id_actividad = actividades.id_actividad ) AS inscritos    
                FROM actividades_paquetes s
        ...
        if ($query->NumeroRegistros() > 0) {
            while ($row = $query->VerRegistro()) {
                $data[] = array(
                    ...
                    'id_paquete'     => $row['id_actividad_paquete'],
