...
//        ?>
    ...
    public function listPaquetesAdmin() {
        ?>
        <div id="content-area">
            Listar por: 
    ...
    public function newEventos() {
        ...
        <div id="steps">
            <div id="titu_step"> <span class="glyphicon glyphicon-calendar"></span>Crear Evento <a class="btn btn_nuevo" href="cuenta.php?actividades=eventos" title="listar Eventos">Regresar</a></div>
            <div id="panel_step">
                ...
                <section  style="display:inline-block; width:67%; padding-top:10px; padding-left:10px ">
                    <form action="cuenta.php?actividades=eventos&action=add" method="post" enctype="multipart/form-data" accept-charset="utf-8" name="form_datos" class="form-horizontal" id="form_datos" onsubmit="return validate2(this, 'add')" >
...
Recomendaciones:
						...
						<!--
                        <div class="form-group"> <input name="reglamento_actividad_evento" id="reglamento_actividad_evento" type="text" class="form-control" placeholder="Reglamento de actividades"></div>
                        -->
    ...
    public function addEventosCuenta(Cliente $cliente){
        ...
        $sql2 = "INSERT INTO actividades_eventos(id_actividad,reglamento_actividad_evento, cierre_inscripciones_actividad_evento)  
                                         VALUES ('" . $id_actividad . "','" . $_POST['reglamento_actividad_evento'] . "','" . $fecha_cierre . "')";
        $query2 = new Consulta($sql2);
        //echo $sql2; 
        ...
        //$this->_msgbox->setMsgbox('Se agregó correctamente.', 2);       
        location("cuenta.php?actividades=eventos");
    ...
    public function editEventosCuenta(){
        settype($_GET['id'],'int');
        $data = new EventoActividad($_GET['id']);        
        ...
        <div id="steps">
            <div id="titu_step"> <span class="glyphicon glyphicon-calendar"></span>Editar Evento <a class="btn btn_nuevo" href="cuenta.php?actividades=eventos" title="Nuevo Evento">Regresar</a></div>
            <div id="panel_step">
               
                <form action="cuenta.php?actividades=eventos&action=update&id=<?php echo $data->_id_salida;?>" method="post" enctype="multipart/form-data" accept-charset="utf-8" name="form_datos" id="form_datos" onsubmit="return validate2(this, 'update')">
                    <input type="hidden" value="update" name="action">
                    <input type="hidden" value="<?php echo $data->__get('_id_evento');?>" name="id_actividad_salida">
                    ...
                    <div id="div_input1">
                        ...
                        <!--
                        <div class="rowElem"><label>Reglamento:</label><input name="reglamento" id="reglamento" type="text" size="80" value="<?php echo $data->__get('_reglamento');?>"  /></div>
                       -->
                        ...
                        <div class="rowElem"><label>Fecha Cierre:</label><input name="fecha_cierre" id="fecha_cierre" class="date" type="text" size="15" style="width:150px;" value="<?php echo implode("-",array_reverse(explode("-", $data->__get('_fecha'))));?>">&nbsp;&nbsp;<img src="aplication/webroot/imgs/icon_cal2.png"/></div>
   ...
   public function updateEventosCuenta(Cliente $cliente){
        ...
        $user = $_SESSION['usuario']; 
            ...
            //$query = new Consulta("UPDATE actividades SET nombre_actividad='" . $_POST['titulo_salida'] . "' WHERE id_actividad = '7' ");
            //$query = new Consulta("UPDATE actividades SET nombre_actividad='" . $_GET['action'] . "' WHERE id_actividad = '7' ");
            ...
            $query2 = new Consulta("UPDATE actividades_eventos SET 
                                    reglamento_actividad_evento='" . $_POST['reglamento'] . "',
                                    cierre_inscripciones_actividad_evento='" . $fecha_cierre . "'
                                    WHERE id_actividad = '" . $_POST['id_actividad'] . "' ");
    ...
    public function stateEventosCuenta(Cliente $cliente){
    ...
    public function deleteEventosCuenta(Cliente $cliente) {

        $query1 = new Consulta("DELETE FROM actividades WHERE id_actividad = '" . $_GET['id'] . "'"); //Son los salidas por cliente Si estará ahi o no
        $query2 = new Consulta("DELETE FROM actividades_eventos WHERE id_actividad_evento = '" . $_GET['id'] . "' ");
    ...
    public function listEventos(Cliente $cliente){
         
        $salidas = $this->getEventosCuenta($cliente->__get("_id"));
        
        $nsalidas = count($salidas);  ?>
        <div id="steps">
            <div id="titu_step"> <span class="glyphicon glyphicon-calendar"></span> Eventos <a class="btn btn_nuevo" href="cuenta.php?actividades=eventos&action=new" title="Nueva Salida">Nuevo Evento</a></div>
            <div id="panel_step">
                ...
                <?php 
                    for($i = 0; $i < $nsalidas; $i++){
                      ...
                      <tr>
                          ...
                          <td> <?php echo $salidas[$i]['id_evento'];?>   </td>
                        ...
                        <td>
                            <a href="cuenta.php?actividades=eventos&action=inscritos&id_actividad=<?php echo $salidas[$i]['id_actividad']?>&id_tipo_actividad=<?php echo $salidas[$i]['id_tipo_actividad']?>" class="btn-circle btn-registro glyphicon glyphicon-th-list" title="Inscritos"> </a>                       
                                           
                            <a href="cuenta.php?actividades=eventos&action=edit&id=<?php echo $salidas[$i]['id_evento']?>" class="btn-circle btn-edit glyphicon glyphicon-pencil" title="Editar"> </a>
                            <!--<a href="cuenta.php?actividades=eventos&action=del&id=<?php echo $salidas[$i]['id_actividad']?>" class="btn-circle btn-delete glyphicon glyphicon-remove" title="Eliminar"></a>-->
                            <a href="cuenta.php?actividades=eventos&action=state&id=<?php echo $salidas[$i]['id_actividad']?>&id_estado=<?php echo $salidas[$i]['estado']?>" class="btn-circle btn-info glyphicon glyphicon-remove-sign" title="Activar/Desactivar"></a>
                            
                            <a href="cuenta.php?actividades=eventos&action=estaditica&id=<?php echo $salidas[$i]['id_evento']?>" class="btn-circle btn-reporte glyphicon glyphicon-stats" title="Estadistica"> </a>   
                        </td>
                      </tr>
    ...
    public function getEventosCuenta($id_cliente){
        $sql = "SELECT id_actividad,id_actividad_evento, id_tipo_actividad, nombre_actividad, estado_actividad, fecha_actividad,
                    ( SELECT COUNT(i.id_inscripcion) FROM inscripciones i  WHERE i.id_actividad = actividades.id_actividad ) AS inscritos
                FROM actividades_eventos s
        ...
        if ($query->NumeroRegistros() > 0) {
            while ($row = $query->VerRegistro()) {
                $data[] = array(
                    ...
                    'id_evento'     => $row['id_actividad_evento'],
