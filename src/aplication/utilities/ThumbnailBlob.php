...
class ThumbnailBlob{
	...
	*/
	...
	function SetTransparencia($transparency){
	...
	*/
	...
	function CreateThumbnail($width=0, $height=0, $fecha=""){		
		...
		if($width > 0 || $height > 0 ){
			...
			if(!empty($this->image)){		
				...
				/* depende de que tipo de imagen sea genero el png(transparencia) ó jpg */
				switch ($this->transparencia){
					case false : imagejpeg($this->thumbnail, $this->path.$fecha, 98);	break;					
					case true :  imagepng($this->thumbnail, $this->path.$fecha, 9); break;
					break;
				}				
...
/**
 * anb
 * Generate thubnail with Inpath, OutPath
 * Note: only create image not Eliminate file
 * @return  Void Create image thumbnails
 */
public static function makeThumbnails($filePathIn, $filePathOut, $thumbnail_width = 100, $thumbnail_height=100)
{
    $arr_image_details = getimagesize($filePathIn); // pass id to thumb name
    ...
    if ($imgt) {
        $old_image = $imgcreatefrom($filePathIn);
